...
public abstract class EventListener {
  ...
  public void responseHeadersEnd(Call call, Response response) {
    boolean enableMarauderRefresh = true;
    if (enableMarauderRefresh) {
      CacheControl responseCacheControl = response.cacheControl();
        boolean isNoCache = responseCacheControl.noCache();
        int maxAgeInSeconds = responseCacheControl.maxAgeSeconds();
        if (!isNoCache && maxAgeInSeconds > 0) {
          System.out.format("MARAUDER: enqueuing. URL:%s. no-cache:%s. max-age:%s\n", response.request().url().url().toString(), isNoCache, maxAgeInSeconds);
          System.out.format("MARAUDER: in event listener for enqueuing above URL. is client null?(T/F) %s\n", this.getClient() == null);
          RealCall scheduledCall = RealCall.newRealCall(this.getClient(), response.request(), false /* for web socket */);
          scheduledCall.setExpiringMaxAge(maxAgeInSeconds);
          this.client.marauder.enqueue(scheduledCall.getAsAsyncCall());
      }
    } 
  }
  ...
  public void callEnd(Call call) {
    System.out.println("XXX IN event listener event call end");
  }
